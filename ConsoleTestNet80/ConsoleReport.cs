...
internal static class ConsoleReport
{
    ...
    private static string RedactSecrets(string? value)
    {
        ...
        // ValidateUserEnDeCode 使用固定规则生成可逆的旧式编码值；该值也必须按密码处理。
        string encodedValidatePassword = EnDecode.Encode(validatePassword);
        if (!string.IsNullOrEmpty(encodedValidatePassword))
        {
            redacted = redacted.Replace(encodedValidatePassword, "******（旧式编码值已脱敏）");
        }
